...
import { Category, ItineraryItem, DailyItinerary, Expense, ShoppingItem, ShoppingType } from './types';
...
const ShoppingSection: React.FC = () => {
  const [items, setItems] = useState<ShoppingItem[]>(() => {
    const saved = localStorage.getItem('trip_shopping_list');
    return saved ? JSON.parse(saved) : [];
  });
  const [activeType, setActiveType] = useState<ShoppingType>(ShoppingType.PERSONAL);
  ...
  const addItem = () => {
    ...
    const newItem: ShoppingItem = {
      ...
      quantity,
      type: activeType,
      owner: activeType === ShoppingType.PROXY ? owner : undefined,
      isPurchased: false,
    };
  ...
  return (
    <div className="px-6 py-10 pb-32 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        <h2 className="serif text-3xl font-bold text-slate-900">購物清單</h2>
        <button 
          onClick={() => setIsAdding(!isAdding)}
          className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-lg ${isAdding ? 'bg-rose-500 text-white rotate-45' : 'bg-slate-900 text-white'}`}
        >
          <Icons.Plus />
        </button>
      </div>

      <div className="bg-slate-100 p-1.5 rounded-[1.5rem] flex mb-8">
        <button 
          onClick={() => setActiveType(ShoppingType.PERSONAL)}
          className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-2xl transition-all ${activeType === ShoppingType.PERSONAL ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
        >
          我的必買
        </button>
        <button 
          onClick={() => setActiveType(ShoppingType.PROXY)}
          className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-2xl transition-all ${activeType === ShoppingType.PROXY ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
        >
          代購任務
        </button>
      </div>
      ...
      {isAdding && (
        <div className="bg-white rounded-[2rem] p-7 mb-8 border border-slate-100 shadow-xl animate-in zoom-in-95 duration-300">
          <h3 className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mb-5">新增項目</h3>
          <div className="space-y-4">
            <input 
              type="text" 
              placeholder="想買什麼？ (如：大眼蝦、化妝水)" 
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-slate-200 outline-none"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
            />
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest pl-1 mb-2 block">數量</label>
                <input 
                  type="number" 
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm"
                  value={quantity}
                  onChange={(e) => setQuantity(parseInt(e.target.value) || 1)}
                />
              </div>
              {activeType === ShoppingType.PROXY && (
                <div className="flex-[2]">
                  <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest pl-1 mb-2 block">委託人</label>
                  <input 
                    type="text" 
                    placeholder="誰要的？"
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm"
            ...
            <button 
              onClick={addItem}
              className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-sm shadow-xl active:scale-95 transition-all mt-2"
            >
              加入清單
            </button>
          </div>
      ...
      <div className="mb-8">
        <div className="flex justify-between items-end mb-2">
          ...
          <p className="text-xs font-black text-slate-900">{purchasedCount} / {filteredItems.length}</p>
        </div>
        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
          <div 
            className="h-full bg-indigo-500 transition-all duration-500 ease-out rounded-full"
            style={{ width: `${filteredItems.length ? (purchasedCount / filteredItems.length) * 100 : 0}%` }}
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredItems.length === 0 ? (
          <div className="text-center py-20 bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-100">
            <p className="text-slate-300 text-sm italic font-medium">清單是空的，準備大掃貨吧！</p>
          </div>
        ) : (
          filteredItems.map(item => (
            <div 
              key={item.id}
              className={`bg-white rounded-[1.75rem] border transition-all ${item.isPurchased ? 'border-transparent opacity-50' : 'border-slate-100 shadow-sm'}`}
            >
              <div className="p-5 flex items-center gap-4">
                <button 
                  onClick={() => togglePurchased(item.id)}
                  className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all ${item.isPurchased ? 'bg-indigo-500 border-indigo-500 text-white' : 'bg-white border-slate-200 text-transparent'}`}
                >
                  <Icons.Check />
                </button>
                <div className="flex-1" onClick={() => togglePurchased(item.id)}>
                  <p className={`font-bold text-slate-800 ${item.isPurchased ? 'line-through' : ''}`}>
                    {item.title}
                    <span className="ml-2 text-indigo-500">x{item.quantity}</span>
                  </p>
                  {item.owner && (
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">委託人: {item.owner}</p>
                  )}
                </div>
                <button 
      ...
      {zoomedPhrase && (
        ...
        >
          ...
          <div className="text-center w-full max-w-sm">
            ...
            <h4 className="serif text-6xl font-black text-slate-900 leading-tight mb-10 break-words">{zoomedPhrase.kr}</h4>
          ...
          <div className="absolute bottom-20 text-slate-300 text-[10px] font-black uppercase tracking-[0.4em]">Tap anywhere to close</div>
        </div>
      )}
    </div>
  );
...
const BudgetSection: React.FC = () => {
  ...
  const memberStats = useMemo(() => {
    return members.map(m => {
      const totalPaidByMe = expenses.filter(e => e.payer === m).reduce((acc, curr) => acc + curr.amount, 0);
      const totalIShouldPay = expenses.reduce((acc, curr) => curr.participants.includes(m) ? acc + (curr.amount / curr.participants.length) : acc, 0);
    ...
    <div className="px-6 py-10 pb-32 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          ...
          className="text-[10px] font-black px-4 py-2 bg-slate-100 rounded-full text-slate-600 uppercase tracking-[0.2em]"
        >
      ...
      {showMemberManager && (
        <div className="bg-white rounded-[2rem] p-7 mb-10 border border-slate-100 shadow-xl animate-in zoom-in-95 duration-300">
          ...
          <div className="flex flex-wrap gap-2.5 mb-6">
            {members.map(m => (
              <span key={m} className="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-2xl flex items-center gap-2">
                {m}
                {members.length > 1 && <button onClick={() => setMembers(members.filter(x => x !== m))} className="opacity-50">×</button>}
              </span>
            ))}
          </div>
          <div className="flex gap-2">
            <input 
              type="text" value={newMemberName} placeholder="成員姓名"
              className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 text-sm"
              onChange={(e) => setNewMemberName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && addMember()}
            />
            <button onClick={addMember} className="bg-slate-900 text-white px-6 py-3 rounded-2xl text-sm font-bold"><Icons.Plus /></button>
          </div>
        </div>
      )}

      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[2.5rem] p-8 mb-10 shadow-2xl relative overflow-hidden">
          ...
          {memberStats.map(stat => (
            <div key={stat.name} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center text-xs font-black">{stat.name[0]}</div>
      ...
      <div className="bg-white rounded-[2.5rem] p-7 mb-10 shadow-sm border border-slate-50">
        ...
        <div className="flex flex-col gap-5">
          <input type="text" placeholder="項目" className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} />
          <input type="number" placeholder="金額 (KRW)" className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} />
          <div className="flex flex-col gap-3">
            <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest pl-1">付款人</label>
            <div className="flex gap-2 overflow-x-auto no-scrollbar">
              {members.map(m => (
                <button key={m} onClick={() => setSelectedPayer(m)} className={`px-5 py-3 rounded-2xl text-xs font-bold transition-all ${selectedPayer === m ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-400'}`}>{m}</button>
              ))}
            </div>
          </div>
          <button onClick={addExpense} className="bg-slate-900 text-white w-full py-5 rounded-[1.5rem] text-sm font-black active:scale-95 transition-all shadow-xl">確定紀錄</button>
        </div>
      </div>

      <div className="space-y-4">
        {expenses.map((expense) => (
          <div key={expense.id} className="bg-white rounded-3xl border border-slate-100 p-6 flex items-center justify-between">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-2xl bg-slate-900 text-white flex items-center justify-center"><Icons.Wallet /></div>
              <div>
                <p className="text-base font-bold text-slate-800">{expense.title}</p>
                <p className="text-[10px] text-slate-400 font-black tracking-widest">{expense.payer} 付款</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <p className="text-base font-black text-slate-700">₩{expense.amount.toLocaleString()}</p>
              <button onClick={() => deleteExpense(expense.id)} className="text-slate-200 hover:text-rose-500"><Icons.Delete /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mb-14">
        ...
        <div className="space-y-6">
          {FLIGHT_INFO.map((flight, idx) => (
            <div key={idx} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-lg overflow-hidden">
              <div className="bg-slate-900 px-7 py-4 flex justify-between items-center text-white text-[10px] font-black uppercase tracking-[0.3em]">
                <span>Jin Air</span>
                <span className="bg-white/10 px-3 py-1 rounded-full">{flight.no}</span>
              </div>
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  <div className="text-center">
                    <p className="text-[10px] text-slate-300 font-black mb-2">Depart</p>
                    <p className="text-3xl font-black text-slate-800">{flight.from.split(' ')[0]}</p>
                  </div>
                  <Icons.Plane />
                  <div className="text-center">
                    <p className="text-[10px] text-slate-300 font-black mb-2">Arrive</p>
                    <p className="text-3xl font-black text-slate-800">{flight.to.split(' ')[0]}</p>
                  </div>
                </div>
                <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest text-center">{flight.date}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mb-14">
        ...
        <div className="space-y-5">
          {Array.from(new Set(TRIP_DATA.map(d => d.hotel.name))).map((hotelName, idx) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-3xl p-7 border border-slate-100 shadow-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black text-slate-400">{hotel.phone}</span>
                  <a href={`tel:${hotel.phone.replace(/-/g, '')}`} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-xs font-bold">Call</a>
                </div>
              </div>
            );
          })}
      ...
      <div className="mb-14">
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 divide-y divide-slate-50">
          {EMERGENCY_CONTACTS.map((contact, idx) => (
            <div key={idx} className="p-7 flex justify-between items-center">
              <div>
              ...
              <a href={`tel:${contact.phone.replace(/-/g, '')}`} className="w-12 h-12 bg-rose-50 text-rose-600 rounded-2xl flex items-center justify-center"><Icons.Phone /></a>
            </div>
          ))}
        </div>
      </div>
...
type AppTab = 'itinerary' | 'info' | 'korean' | 'notes' | 'budget' | 'shopping';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] max-w-md mx-auto relative overflow-x-hidden pb-10">
      {activeTab === 'itinerary' && (
        <div className="px-6 pt-12 mb-8 animate-in fade-in duration-700">
          <div className="flex justify-between items-start mb-10">
            ...
            <div className="pt-2"><div className="w-12 h-12 bg-white rounded-2xl border border-slate-100 flex items-center justify-center text-xl shadow-sm">❄️</div></div>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
            {TRIP_DATA.map((day) => (
              <button key={day.day} onClick={() => setSelectedDay(day.day)} className={`flex-shrink-0 w-[4.5rem] h-[5rem] rounded-[1.75rem] flex flex-col items-center justify-center transition-all ${selectedDay === day.day ? 'bg-slate-900 text-white shadow-xl' : 'bg-white text-slate-400 border border-slate-100'}`}>
          ...
          <div className="px-6 pb-32 animate-in fade-in slide-in-from-bottom-6 duration-700">
            <div className="mb-8">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-xl font-black text-slate-800">{currentDayData.date} ({currentDayData.weekday})</h2>
            ...
            <div className="relative pl-7">
              ...
              {currentDayData.items.map((item) => (
                <div key={item.id} className="relative mb-8 last:mb-0">
                  <div className="absolute -left-[27px] top-6 w-3 h-3 rounded-full bg-white border-[3px] border-slate-900 z-10" />
                  <ItineraryCard item={item} />
                </div>
              ))}
            </div>
            <div className="mt-12 pt-10 border-t border-slate-100">
              <h3 className="text-[10px] text-slate-300 font-black uppercase tracking-[0.3em] mb-6">Tonight Stay</h3>
              <div className="bg-white rounded-[2rem] p-7 border border-slate-100">
                ...
                <p className="text-xs text-slate-400 mb-5">{currentDayData.hotel.address}</p>
                {currentDayData.hotel.phone !== '-' && (
                  <a href={`tel:${currentDayData.hotel.phone.replace(/-/g, '')}`} className="block w-full bg-slate-900 text-white text-center py-3.5 rounded-2xl text-xs font-bold shadow-lg">聯繫櫃檯</a>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[94%] max-w-sm bg-white/80 backdrop-blur-2xl border border-white/40 shadow-2xl rounded-[2.5rem] px-1 py-3 flex items-center justify-between z-[90] animate-in slide-in-from-bottom-10">
        {[
          { id: 'itinerary', icon: <Icons.Map />, label: '行程' },
          { id: 'info', icon: <Icons.Info />, label: '資訊' },
          { id: 'shopping', icon: <Icons.Shopping />, label: '購物' },
          { id: 'korean', icon: <Icons.Translate />, label: '翻譯' },
          { id: 'notes', icon: <Icons.Note />, label: '錦囊' },
          { id: 'budget', icon: <Icons.Wallet />, label: '預算' }
        ].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as AppTab)} className={`flex flex-col items-center flex-1 transition-all ${activeTab === tab.id ? 'text-slate-900' : 'text-slate-300'}`}>
            <div className={`p-1.5 rounded-xl ${activeTab === tab.id ? 'bg-slate-900 text-white shadow-lg' : ''}`}>{tab.icon}</div>
            <span className={`text-[7px] mt-1 font-black uppercase tracking-widest ${activeTab === tab.id ? 'opacity-100' : 'opacity-0 scale-50'}`}>{tab.label}</span>
          </button>
        ))}
      </div>
